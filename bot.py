    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if bet <= 0:
                ...
                return
            if bet > 100000:
                await interaction.response.send_message("賭け金の上限は 100,000 コインです。", ephemeral=True)
                return
            ...
            if last_date != today_str:
                # 日付が変わっていたらリセット
                await database.reset_chinchiro_count(interaction.user.id, today_str)
                count = 0
            
            if count >= 10:
                await interaction.response.send_message("チンチロリンは1日10回までです。また明日挑戦してください！", ephemeral=True)
                return
            ...
            # 回数カウントアップ
            await database.increment_chinchiro_count(interaction.user.id)
...
class CoinflipBetModal(discord.ui.Modal, title='コイントス：賭け金入力'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # 表裏選択ビューを表示
            view = CoinflipGameView(interaction.user, bet)
            await interaction.response.send_message(
                f"🪙 **コイントス勝負！**\n賭け金: **{bet} {CURRENCY_NAME}**\n「表」か「裏」か選んでください！",
                view=view,
                ephemeral=True
