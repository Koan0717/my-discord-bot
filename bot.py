...
DEFAULT_SETTINGS = {
    ...
    "GAMBLE_MAX_PLAYS": 10,
    "GAMBLE_WIN_RATE_CHINCHIRO": -1,
    "GAMBLE_WIN_RATE_COINFLIP": -1,
    "GAMBLE_WIN_RATE_SLOT": -1,
    "GAMBLE_WIN_RATE_BLACKJACK": -1,
    "GAMBLE_WIN_RATE_ROULETTE": -1
}
...
def has_gamble_employee_role(user: discord.Member):
    employee_role_ids = get_setting("GAMBLE_EMPLOYEE_ROLE_IDS") or []
    admin_role_ids = get_setting("ADMIN_ROLE_IDS") or []
    user_role_ids = [role.id for role in user.roles]
...
def has_evaluator_role(user: discord.Member):
    eval_role_ids = get_setting("EVALUATOR_ROLE_IDS") or []
    admin_role_ids = get_setting("ADMIN_ROLE_IDS") or []
    user_role_ids = [role.id for role in user.roles]
...
def has_interviewer_role(user: discord.Member):
    interviewer_role_ids = get_setting("INTERVIEWER_ROLE_IDS") or []
    admin_role_ids = get_setting("ADMIN_ROLE_IDS") or []
...
def is_free_inn_member(user: discord.Member):
    free_inn_role_ids = get_setting("FREE_INN_ROLE_IDS") or []
    user_role_ids = [r.id for r in user.roles]
...
def is_rank_eligible(channel) -> bool:
...
class EconomyBot(commands.Bot):
    ...
    def get_rank_config(self, guild_id: int) -> dict:
        """キャッシュからランク設定を返す。未キャッシュの場合は空の設定を返す。"""
    ...
    async def setup_hook(self):
        ...
        global CURRENCY_NAME, INITIAL_COINS
        CURRENCY_NAME = self.bot_settings.get("CURRENCY_NAME", "コイン")
        INITIAL_COINS = self.bot_settings.get("INITIAL_COINS", 30000)
        ...
        self.tree.add_command(GamblingEmployeeGroup())
    ...
    async def vc_reward_loop(self):
        ...
        for user_id, last_reward_time in list(self.vc_sessions.items()):
            ...
            if member:
                # ランク対象判定および評価時間対象カテゴリー判定
                in_correct_category = is_rank_eligible(member.voice.channel)
                eval_category_id = get_setting("EVALUATION_CATEGORY_ID")
                is_eval_category = (member.voice.channel.category and member.voice.channel.category.id == eval_category_id)
                
                if not in_correct_category and not is_eval_category:
                    # どちらの条件も満たしていない場合はセッションを終了
...
async def on_message(message):
    if message.guild:
        ...
        if sticky_data:
            # 自分が送信した直後のテンプレメッセージ自身ならスキップ
            if not (message.author.id == bot.user.id and message.id == sticky_data["last_message_id"]):
                if sticky_data["last_message_id"]:
                    try:
                        ...
                        pass
                embed = discord.Embed(title=sticky_data["title"], description=f"```\n{sticky_data['content']}\n```", color=discord.Color.orange())
                new_msg = await message.channel.send(embed=embed)
                await database.update_sticky_last_message(message.channel.id, new_msg.id)
    ...
    # 2. TC経験値の判定
    # ランク対象判定
    in_correct_category = is_rank_eligible(message.channel)
...
async def on_voice_state_update(member, before, after):
    ...
    try:
        ...
        if after.channel is not None:
            ...
            if is_join:
                # ランク対象または評価対象カテゴリーの判定を満たす場合のみセッション開始
                in_correct_category = is_rank_eligible(after.channel)
                eval_category_id = get_setting("EVALUATION_CATEGORY_ID")
                is_eval_category = (after.channel.category and after.channel.category.id == eval_category_id)
                
                if in_correct_category or is_eval_category:
                    print(f"[VC XP] Started session for {member.display_name} (rank={in_correct_category}, eval={is_eval_category})")
        ...
        if before.channel is not None and (after.channel is None or before.channel != after.channel):
            ...
            if join_time:
                ...
                if duration_minutes > 0:
                    # XP付与は退出したチャンネルがランク対象だった場合のみ実行
                    if is_rank_eligible(before.channel):
...
def is_gamble_employee():
    async def predicate(interaction: discord.Interaction):
        if not interaction.guild:
            return False
        if has_gamble_employee_role(interaction.user):
            return True
...
async def rank(interaction: discord.Interaction, user: discord.Member = None):
    ...
    try:
        ...
        if eval_category_id and eval_category_id != 123456789012345678:
            ...
            active_extra = 0
            if target_user.id in bot.vc_sessions and target_user.voice and target_user.voice.channel and target_user.voice.channel.category:
                if target_user.voice.channel.category.id == eval_category_id:
                    join_time = bot.vc_sessions[target_user.id]
...
class ChinchiroBetModal(discord.ui.Modal, title='チンチロリン：賭け金入力'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            max_bet = get_setting("GAMBLE_MAX_BET") or 100000
            bet = int(self.bet_input.value)
            if bet <= 0 or bet > max_bet: return await interaction.response.send_message(f"1〜{max_bet:,}の範囲で入力してください。", ephemeral=True)
            await interaction.response.defer(ephemeral=True)
            ...
            max_plays = get_setting("GAMBLE_MAX_PLAYS") or 10
            if count >= max_plays: return await interaction.followup.send("本日の上限に達しました。", ephemeral=True)
            ...
            await interaction.followup.send(f"🎲 **チンチロリン開始！** (本日 {count+1}/{max_plays}回目)\n賭け金: **{bet} {CURRENCY_NAME}**", view=view, ephemeral=True)
...
class ChinchiroGameView(discord.ui.View):
    ...
    def get_rank_and_score(self, d):
        d.sort()
        if d == [1,1,1]: return "ピンゾロ", 1000, 5
        if d == [2,2,2]: return "アラシ(2)", 900, 3
        if d == [3,3,3]: return "アラシ(3)", 800, 3
        if d == [4,4,4]: return "アラシ(4)", 700, 3
        if d == [5,5,5]: return "アラシ(5)", 600, 3
        if d == [6,6,6]: return "アラシ(6)", 500, 3
        if d == [4,5,6]: return "シゴロ", 400, 2
        if d == [1,2,3]: return "ヒフミ", -100, -2
        if d[0]==d[1]: return f"出目{d[2]}", 100+d[2], 1
        if d[1]==d[2]: return f"出目{d[0]}", 100+d[0], 1
        if d[0]==d[2]: return f"出目{d[1]}", 100+d[1], 1
        return "役なし", 0, 1
    ...
    async def roll(self, interaction, button):
        ...
        self.player_rolls += 1
        pd = [random.randint(1,6) for _ in range(3)]
    ...
    async def run_npc_turn(self, interaction):
        ...
        forced_win = get_forced_outcome("CHINCHIRO")
        
        while npc_rolls < 3:
            npc_rolls += 1
            # Generate outcome matching forced state
            for _ in range(50): # Max attempts to prevent hang
                bd = [random.randint(1,6) for _ in range(3)]
                bh, br, bm = self.get_rank_and_score(bd)
                if forced_win is True:
                    if br < self.player_rank or br == 1: break
                elif forced_win is False:
                    if br > self.player_rank or br == 15: break
                else:
                    break
        ...
        if win_status == "draw":
            await database.add_balance(self.user.id, self.bet)
            ...
            win_amount = self.bet * win_mul
            await database.add_balance(self.user.id, self.bet + win_amount)
            res = f"🏆 勝ち！ {win_amount} {CURRENCY_NAME} 獲得！"
            ...
            if extra_loss > 0:
                current_bal = await database.get_balance(self.user.id)
                if current_bal >= extra_loss:
                    await database.remove_balance(self.user.id, extra_loss)
                    res = f"💀 負け… (合計 {self.bet * loss_mul} {CURRENCY_NAME} 没収)"
                else:
                    await database.remove_balance(self.user.id, current_bal)
...
class CoinflipBetModal(discord.ui.Modal, title='コイントス：賭け金入力'):
    ...
    async def on_submit(self, it: discord.Interaction):
        try:
            ...
            max_plays = get_setting("GAMBLE_MAX_PLAYS") or 10
            bet = int(self.bet_input.value)
            if bet <= 0 or bet > max_bet: return await it.response.send_message(f"1〜{max_bet:,}の範囲で入力してください。", ephemeral=True)
...
class CoinflipGameView(discord.ui.View):
    ...
    async def process(self, it, choice):
        ...
        forced_win = get_forced_outcome("COINFLIP")
        if forced_win is True:
            res = choice
        elif forced_win is False:
...
class SlotBetModal(discord.ui.Modal, title='スロット：賭け金入力'):
    ...
    async def on_submit(self, it: discord.Interaction):
        try:
            ...
            forced_win = get_forced_outcome("SLOT")
            if forced_win is True:
                r = [random.choice(emo)] * 3
            elif forced_win is False:
...
class BlackjackBetModal(discord.ui.Modal, title='ブラックジャック：賭け金入力'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            max_plays = get_setting("GAMBLE_MAX_PLAYS") or 10
            if count >= max_plays:
                return await interaction.followup.send("本日の上限に達しました。", ephemeral=True)
...
class BlackjackGameView(discord.ui.View):
    ...
    async def resolve_stand(self, interaction: discord.Interaction = None):
        ...
        player_score = calculate_blackjack_score(self.player_hand)
        forced_win = get_forced_outcome("BLACKJACK")
...
async def run_roulette_game(interaction: discord.Interaction, user, bet, count, bet_type, target_num):
    ...
    max_plays = get_setting("GAMBLE_MAX_PLAYS") or 10
    if current_count >= max_plays:
        try:
            await interaction.followup.send("本日の上限に達しました。", ephemeral=True)
        except Exception:
            await interaction.response.send_message("本日の上限に達しました。", ephemeral=True)
        return
    ...
    forced_win = get_forced_outcome("ROULETTE")
    if forced_win is True:
...
class EconomySettingsModal(discord.ui.Modal, title="経済設定"):
    def __init__(self, current_currency: str, current_coins: int):
        super().__init__()
        ...
        self.initial_coins = discord.ui.TextInput(
            label="初期所持金",
            style=discord.TextStyle.short,
            default=str(current_coins),
            required=True,
            max_length=15
        )
        self.add_item(self.currency_name)
        self.add_item(self.initial_coins)

    async def on_submit(self, interaction: discord.Interaction):
        try:
            new_coins = int(self.initial_coins.value)
        except ValueError:
            return await interaction.response.send_message("❌ 初期所持金は整数で入力してください。", ephemeral=True)
            
        await database.save_setting("CURRENCY_NAME", self.currency_name.value)
        await database.save_setting("INITIAL_COINS", new_coins)
        
        interaction.client.bot_settings["CURRENCY_NAME"] = self.currency_name.value
        interaction.client.bot_settings["INITIAL_COINS"] = new_coins
        
        global CURRENCY_NAME, INITIAL_COINS
        CURRENCY_NAME = self.currency_name.value
        INITIAL_COINS = new_coins

        await update_main_admin_panel(interaction, interaction.client)
...
class ManageEconomySettingsButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            style=discord.ButtonStyle.secondary,
            emoji="💰",
            custom_id="persistent_admin_manage_economy_settings_btn"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        global CURRENCY_NAME, INITIAL_COINS
        modal = EconomySettingsModal(CURRENCY_NAME, INITIAL_COINS)
        await interaction.response.send_modal(modal)
...
async def update_rank_settings_config_view(interaction: discord.Interaction, bot):
    cfg = await bot.fetch_and_cache_rank_config(interaction.guild_id)
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**【判定ルール】**\n"
            "1. **有効が指定されてない場合**: 無効にしたチャンネル・カテゴリー**以外**すべてが対象になります。\n"
            "2. **無効が指定されていない場合**: 有効にしたチャンネル・カテゴリー**のみ**が対象になります。\n"
            "3. **両方指定されている場合**: 有効リストに含まれ、かつ無効リストに含まれないチャンネル・カテゴリーが対象になります。\n"
            "4. **どちらも指定がない場合**: すべてのチャンネルが対象になります。\n\n"
            "**【設定手順】**\n"
            "- 各ドロップダウンメニューから、有効/無効チャンネル、または対象カテゴリーを選択（各最大25個）できます。\n"
    ...
    whitelist_ch_str = ", ".join(whitelist_strs) if whitelist_strs else "未設定 (無効以外が対象)"

    whitelist_cat_strs = []
    for cid in cfg.get("categories", set()):
        ch = interaction.guild.get_channel(cid)
        if ch:
            whitelist_cat_strs.append(ch.name)
        else:
            whitelist_cat_strs.append(f"⚠️ 不明なカテゴリー (ID: `{cid}`)")
    whitelist_cat_str = ", ".join(whitelist_cat_strs) if whitelist_cat_strs else "未設定"
    ...
    blacklist_ch_str = ", ".join(blacklist_strs) if blacklist_strs else "未設定"

    blacklist_cat_strs = []
    for cid in cfg.get("blacklist_categories", set()):
        ch = interaction.guild.get_channel(cid)
        if ch:
            blacklist_cat_strs.append(ch.name)
        else:
            blacklist_cat_strs.append(f"⚠️ 不明なカテゴリー (ID: `{cid}`)")
    blacklist_cat_str = ", ".join(blacklist_cat_strs) if blacklist_cat_strs else "未設定"

    embed.add_field(
        name="現在の設定",
        value=(
            f"• **有効チャンネル**: {whitelist_ch_str}\n"
            f"• **有効カテゴリー (ランク対象)**: {whitelist_cat_str}\n"
            f"• **無効チャンネル**: {blacklist_ch_str}\n"
            f"• **無効カテゴリー**: {blacklist_cat_str}"
        ),
        inline=False
    )
...
class WhitelistChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await database.set_rank_settings(interaction.guild_id, list(cfg["whitelist"]), list(cfg["blacklist"]), list(cfg.get("categories", [])), list(cfg.get("blacklist_categories", [])))
        await update_rank_settings_config_view(interaction, bot)

class WhitelistCategorySelect(discord.ui.ChannelSelect):
    def __init__(self, bot, guild_id):
        super().__init__(
            placeholder="有効にするカテゴリーを選択/変更...",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await database.set_rank_settings(interaction.guild_id, list(cfg["whitelist"]), list(cfg["blacklist"]), list(cfg["categories"]), list(cfg.get("blacklist_categories", [])))
        await update_rank_settings_config_view(interaction, bot)

class BlacklistChannelSelect(discord.ui.ChannelSelect):
    def __init__(self, bot, guild_id):
        super().__init__(
            placeholder="無効にするチャンネルを選択/変更...",
            channel_types=[
                discord.ChannelType.text,
                discord.ChannelType.voice,
                discord.ChannelType.news,
...
class BlacklistCategorySelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await database.set_rank_settings(interaction.guild_id, list(cfg["whitelist"]), list(cfg["blacklist"]), list(cfg.get("categories", [])), list(cfg["blacklist_categories"]))
        await update_rank_settings_config_view(interaction, bot)
...
class ClearBlacklistCatButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await database.set_rank_settings(interaction.guild_id, list(cfg["whitelist"]), list(cfg["blacklist"]), list(cfg["categories"]), list(cfg["blacklist_categories"]))
        await update_rank_settings_config_view(interaction, bot)
...
async def update_main_admin_panel(interaction: discord.Interaction, bot):
    ...
    # ロール名解決ヘルパー関数
    def resolve_role_name(guild, role_id_key, fallback_name):
        """設定からロールIDを取得して実際のロール名を返す。未設定の場合は fallback_name を返す。"""
        role_id = get_setting(role_id_key)
        if role_id and role_id != 123456789012345678:
            role = guild.get_role(role_id)
            if role:
                return role.mention
        return f"**{fallback_name}** *(未設定)*"

    def resolve_role_names_list(guild, role_ids_key, fallback_names):
        """設定からロールIDリストを取得して実際のロール名リストを返す。"""
        role_ids = get_setting(role_ids_key) or []
        if role_ids:
            names = []
            for rid in role_ids:
                role = guild.get_role(rid)
            ...
            return ", ".join(names)
        return ", ".join([f"**{n}** *(未設定)*" for n in fallback_names])

    guild = interaction.guild

    # 無料宿ロール名を解決
    free_inn_role_ids = get_setting("FREE_INN_ROLE_IDS") or []
    if free_inn_role_ids:
        free_inn_role_str = ", ".join(
            [role.mention if (role := guild.get_role(rid)) else f"`{rid}`" for rid in free_inn_role_ids]
        )
    else:
        free_inn_role_str = ", ".join([f"**{n}** *(未設定)*" for n in FREE_INN_ROLE_NAMES])

    # 宿・カスタムVC
    embed.add_field(
        name="🏨 部屋・宿設定",
        value=(
            f"🛖 **一般宿** (無料ロール: {free_inn_role_str}):\n"
    ...
    # ロール・メンバーシップ（実際のロール名を解決）
    new_member_role_str = resolve_role_name(guild, "NEW_MEMBER_ROLE_ID", NEW_MEMBER_ROLE_NAME)
    pending_member_role_str = resolve_role_name(guild, "PENDING_MEMBER_ROLE_ID", PENDING_MEMBER_ROLE_NAME)
    interviewer_role_str = resolve_role_names_list(guild, "INTERVIEWER_ROLE_IDS", INTERVIEWER_ROLE_NAMES)
    embed.add_field(
        name="👥 ロール設定",
        value=(
            f"入界後ロール: {new_member_role_str}\n"
            f"待機者ロール: {pending_member_role_str}\n"
            f"面接官ロール: {interviewer_role_str}"
        ),
        inline=False
    )

    # 制作・告解（実際のロール名を解決）
    emblem_manager_str = resolve_role_name(guild, "EMBLEM_MANAGER_ROLE_ID", EMBLEM_MANAGER_ROLE_NAME)
    emblem_master_str = resolve_role_name(guild, "EMBLEM_MASTER_ROLE_ID", EMBLEM_MASTER_ROLE_NAME)
    confession_priest_str = resolve_role_name(guild, "CONFESSION_PRIEST_ROLE_ID", CONFESSION_PRIEST_ROLE_NAME)
    priest_str = resolve_role_name(guild, "PRIEST_ROLE_ID", PRIEST_ROLE_NAME)
    embed.add_field(
        name="🎨 制作・告解設定",
        value=(
            f"紋章師ロール: {emblem_manager_str}, {emblem_master_str}\n"
            f"司祭ロール: {confession_priest_str}, {priest_str}"
        ),
        inline=False
    )
    ...
    # レベルロール報酬設定・ランク設定を並列取得
    rewards, rank_cfg = await asyncio.gather(
        database.get_level_role_rewards(),
        bot.fetch_and_cache_rank_config(interaction.guild_id)
    )
    rewards_str = ""
    for r in rewards:
        ltype = "💬 TC" if r["level_type"] == "tc" else "🎙️ VC"
    ...
    # ランク設定 (TC/VC XP対象) ※上のasyncio.gatherで取得済み
    whitelist_strs = []
    ...
    if interaction.response.is_done():
        if interaction.message:
            # ボタン経由（BackToAdminPanelButtonなど）: パネルのメッセージを直接編集
            await interaction.followup.edit_message(message_id=interaction.message.id, embed=embed, view=view)
        else:
            # コマンド経由でdeferした場合: 元のコマンド応答（ephemeral）を編集
...
class BotSetupMainSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="📺 評価時間対象カテゴリー", value="EVALUATION_CATEGORY_ID", description="評価浮上時間のカウント対象となるカテゴリー"),
            ...
            discord.SelectOption(label="👥 賭博従業員ロール", value="GAMBLE_EMPLOYEE_ROLE_IDS", description="賭博従業員のロールを設定します")
        ]
...
class BotSetupMainView(discord.ui.View):
    ...
    def build_embed(self, guild):
        ...
        roles_text = (
            ...
            f"• 賭博従業員: {format_setting_status(guild, 'GAMBLE_EMPLOYEE_ROLE_IDS')}"
        )
        ...
        channels_text = (
            ...
            f"• 評価時間対象カテゴリ: {format_setting_status(guild, 'EVALUATION_CATEGORY_ID')}\n"
...
class StickyTemplateModal(discord.ui.Modal, title='常設テンプレートの作成'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await database.update_sticky_last_message(channel_id, msg.id)
...
class AdminGroup(app_commands.Group):
    ...
    @app_commands.command(name="通貨名設定", description="【管理者専用】通貨の名前を設定・変更します")
    @is_admin()
    async def set_currency_name(self, interaction: discord.Interaction, name: str):
        await database.save_setting("CURRENCY_NAME", name)
        interaction.client.bot_settings["CURRENCY_NAME"] = name
        global CURRENCY_NAME
        CURRENCY_NAME = name
        await interaction.response.send_message(f"✅ 通貨の名前を **{name}** に変更しました。")
        ...
        for member, desired_name in target_users.items():
            ...
            try:
                ...
                if nick_success:
                    ...
                    results.append(f"✅ {member.mention} -> **{desired_name}** (名前変更は権限不足のためスキップされました)")
            except Exception as e:
                results.append(f"❌ {member.display_name} -> 権限エラー等 ({e})")
...
class EvaluationGroup(app_commands.Group):
    ...
    async def list_periods(self, interaction: discord.Interaction):
        ...
        bot = interaction.client
        eval_category_id = get_setting("EVALUATION_CATEGORY_ID")
        evaluation_categories = [eval_category_id] if (eval_category_id and eval_category_id != 123456789012345678) else []
        ...
        for p in periods:
            ...
            # 滞在時間の計算
            stay_seconds = await database.get_vc_duration_for_categories(p['user_id'], evaluation_categories)
            active_extra = 0
            if member and member.id in bot.vc_sessions and member.voice and member.voice.channel and member.voice.channel.category:
                cat_id = member.voice.channel.category.id
                if cat_id in evaluation_categories:
                    join_time = bot.vc_sessions[member.id]
                    active_extra = int((datetime.datetime.now(JST) - join_time).total_seconds())
            ...
            embed.add_field(
                name=name,
                value=f"終了予定: {end_str} (<t:{int(p['end_time'].timestamp())}:R>)\n評価対象VC滞在時間: **{duration_str}**",
                inline=False
            )
    ...
    async def check_period(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        # 滞在時間の計算
        stay_seconds = await database.get_vc_duration_for_categories(target.id, evaluation_categories)
        active_extra = 0
        if target.id in bot.vc_sessions and target.voice and target.voice.channel and target.voice.channel.category:
            cat_id = target.voice.channel.category.id
            if cat_id in evaluation_categories:
                join_time = bot.vc_sessions[target.id]
                active_extra = int((datetime.datetime.now(JST) - join_time).total_seconds())
        ...
        embed.add_field(name="評価対象VC滞在時間", value=f"**{duration_str}**", inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
# --- 賭博従業員専用コマンド ---
def get_forced_outcome(game_name: str):
    # game_name: "CHINCHIRO", "COINFLIP", "SLOT", "BLACKJACK", "ROULETTE"
    rate = get_setting(f"GAMBLE_WIN_RATE_{game_name}")
    if rate is None or rate < 0:
        return None # Natural outcome
    return random.randint(1, 100) <= rate

class GamblingEmployeeGroup(app_commands.Group):
    def __init__(self):
        super().__init__(name="賭博従業員", description="賭博従業員専用コマンド")

    @app_commands.command(name="設定", description="【従業員専用】ギャンブルの設定パネルを表示します")
    @is_gamble_employee()
    async def setup(self, interaction: discord.Interaction):
        embed = discord.Embed(
            title="🎲 賭博従業員用 設定パネル",
            description="各種上限の変更や、プレイヤーの回数制限解除、ゲームの設定が行えます。",
            color=discord.Color.dark_red()
        )
        max_bet = get_setting("GAMBLE_MAX_BET") or 100000
        max_plays = get_setting("GAMBLE_MAX_PLAYS") or 10
        embed.add_field(name="現在の設定", value=f"・1日の金額上限: {max_bet:,}\n・1日のプレイ回数上限: {max_plays}回", inline=False)
        
        await interaction.response.send_message(embed=embed, view=GambleEmployeeSetupView(), ephemeral=True)

class GambleMaxBetModal(discord.ui.Modal, title='1日の金額上限変更'):
    bet_input = discord.ui.TextInput(label='新しい上限金額', placeholder='例: 100000', max_length=10, required=True)
    async def on_submit(self, interaction: discord.Interaction):
        try:
            val = int(self.bet_input.value)
            if val <= 0: raise ValueError
            await database.save_setting("GAMBLE_MAX_BET", val)
            interaction.client.bot_settings["GAMBLE_MAX_BET"] = val
            await interaction.response.send_message(f"金額上限を {val:,} に変更しました。", ephemeral=True)
        except:
            await interaction.response.send_message("正の整数を入力してください。", ephemeral=True)

class GambleMaxPlaysModal(discord.ui.Modal, title='1日のプレイ回数変更'):
    plays_input = discord.ui.TextInput(label='新しい上限回数', placeholder='例: 10', max_length=5, required=True)
    async def on_submit(self, interaction: discord.Interaction):
        try:
            val = int(self.plays_input.value)
            if val <= 0: raise ValueError
            await database.save_setting("GAMBLE_MAX_PLAYS", val)
            interaction.client.bot_settings["GAMBLE_MAX_PLAYS"] = val
            await interaction.response.send_message(f"プレイ回数上限を {val}回 に変更しました。", ephemeral=True)
        except:
            await interaction.response.send_message("正の整数を入力してください。", ephemeral=True)

class GambleResetPlayerSelect(discord.ui.UserSelect):
    def __init__(self):
        super().__init__(placeholder="プレイヤーを選択...", max_values=1)
    async def callback(self, interaction: discord.Interaction):
        target = self.values[0]
        now = datetime.datetime.now(JST)
        today_str = now.strftime("%Y-%m-%d")
        await database.reset_gambling_count(target.id, today_str)
        await interaction.response.send_message(f"{target.mention} の本日のプレイ回数制限を解除しました。", ephemeral=True)

class GambleResetPlayerView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=60)
        self.add_item(GambleResetPlayerSelect())

class GambleWinRateModal(discord.ui.Modal):
    def __init__(self, game):
        super().__init__(title=f'{game} の勝率設定')
        self.game = game
        self.rate_input = discord.ui.TextInput(
            label='勝率 (0〜100、-1で自然確率)',
            placeholder='例: 30',
            max_length=4,
            required=True
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            val = int(self.rate_input.value)
            if val < -1 or val > 100: raise ValueError
            
            key = f"GAMBLE_WIN_RATE_{self.game}"
            await database.save_setting(key, val)
            interaction.client.bot_settings[key] = val
            
            rate_text = f"{val}%" if val >= 0 else "自然確率"
            await interaction.response.send_message(f"✅ {self.game} の勝率を **{rate_text}** に設定しました。", ephemeral=True)
        except:
            await interaction.response.send_message("エラー: -1 から 100 の整数を入力してください。", ephemeral=True)

class GambleSetWinRateView(discord.ui.View):
    def __init__(self, game):
        super().__init__(timeout=60)
        self.game = game
    @discord.ui.button(label="勝率を変更する", style=discord.ButtonStyle.primary)
    async def change_rate(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleWinRateModal(self.game))

class GambleGameSettingsSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="チンチロリン", value="CHINCHIRO", emoji="🎲"),
            discord.SelectOption(label="コイントス", value="COINFLIP", emoji="🪙"),
            discord.SelectOption(label="スロット", value="SLOT", emoji="🎰"),
            discord.SelectOption(label="ブラックジャック", value="BLACKJACK", emoji="🃏"),
            discord.SelectOption(label="ルーレット", value="ROULETTE", emoji="🎡")
        ]
        super().__init__(placeholder="ゲームを選択...", options=options)
    async def callback(self, interaction: discord.Interaction):
        game = self.values[0]
        rate = get_setting(f"GAMBLE_WIN_RATE_{game}")
        rate_text = f"{rate}%" if rate is not None and rate >= 0 else "自然確率（ランダム）"
        
        embed = discord.Embed(
            title=f"🎰 {game} の設定",
            description=f"現在の勝率（プレイヤー側）: **{rate_text}**\n\n勝率を変更する場合は下のボタンを押してください。\n（-1を設定すると自然確率に戻ります）",
            color=discord.Color.blue()
        )
        await interaction.response.edit_message(content=None, embed=embed, view=GambleSetWinRateView(game))

class GambleGameSettingsView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=60)
        self.add_item(GambleGameSettingsSelect())

class GambleEmployeeSetupView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=300)
        
    @discord.ui.button(label="1日の金額上限変更", style=discord.ButtonStyle.primary, row=0)
    async def change_max_bet(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleMaxBetModal())
        
    @discord.ui.button(label="1日のプレイ回数変更", style=discord.ButtonStyle.primary, row=0)
    async def change_max_plays(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleMaxPlaysModal())
        
    @discord.ui.button(label="プレイヤー制限解除", style=discord.ButtonStyle.danger, row=0)
    async def reset_player_limit(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("制限を解除するプレイヤーを選択してください:", view=GambleResetPlayerView(), ephemeral=True)

    @discord.ui.button(label="ギャンブルの設定", style=discord.ButtonStyle.success, row=1)
    async def game_settings(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("設定するゲームを選択してください:", view=GambleGameSettingsView(), ephemeral=True)
