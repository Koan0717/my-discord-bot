...
class ExtendInnSelectView(discord.ui.View):
    ...
    @discord.ui.button(label="12時間 (10,000 Rune)", style=discord.ButtonStyle.success)
    async def twelve(self, interaction: discord.Interaction, button: discord.ui.Button):
        await process_room_extension(interaction, "宿", 12, 10000)
        
    @discord.ui.button(label="24時間 (15,000 Rune)", style=discord.ButtonStyle.success)
    async def twenty_four(self, interaction: discord.Interaction, button: discord.ui.Button):
        await process_room_extension(interaction, "宿", 24, 15000)
...
class ExtendLuxuryInnSelectView(discord.ui.View):
    ...
    @discord.ui.button(label="12時間 (150,000 Rune)", style=discord.ButtonStyle.success)
    async def twelve(self, interaction: discord.Interaction, button: discord.ui.Button):
        await process_room_extension(interaction, "高級宿", 12, 150000)
        
    @discord.ui.button(label="24時間 (250,000 Rune)", style=discord.ButtonStyle.success)
    async def twenty_four(self, interaction: discord.Interaction, button: discord.ui.Button):
        await process_room_extension(interaction, "高級宿", 24, 250000)
...
async def handle_extend(interaction: discord.Interaction):
    ...
    user_roles = [r.name for r in interaction.user.roles]
    is_free_inn = room_type == "宿" and any(r in FREE_INN_ROLE_NAMES for r in user_roles)
    ...
    if room_type == "宿":
        ...
        await process_room_extension(interaction, "カスタムVC", 24, 30000)
...
class InnDurationSelectView(discord.ui.View):
    ...
    @discord.ui.button(label="12時間 (10,000 Rune)", style=discord.ButtonStyle.success, emoji="🛖")
    ...
    @discord.ui.button(label="24時間 (15,000 Rune)", style=discord.ButtonStyle.success, emoji="🛖")
...
class LuxuryInnDurationSelectView(discord.ui.View):
    ...
    @discord.ui.button(label="12時間 (150,000 Rune)", style=discord.ButtonStyle.success, emoji="🏰")
    async def twelve_hours(self, interaction: discord.Interaction, button: discord.ui.Button):
        await process_room_purchase(interaction, "高級宿", 12)

    @discord.ui.button(label="24時間 (250,000 Rune)", style=discord.ButtonStyle.success, emoji="🏰")
    async def twenty_four_hours(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class CustomRoomConfirmView(discord.ui.View):
    ...
    @discord.ui.button(label="確定 (24時間 / 30,000 Rune)", style=discord.ButtonStyle.success, emoji="✅")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class RoomView(discord.ui.View):
    ...
    async def inn(self, it, btn):
        ...
        is_free = any(r in FREE_INN_ROLE_NAMES for r in user_roles)
        if is_free:
...
class CustomRoomView(discord.ui.View):
    ...
    async def custom(self, it, btn): 
        await it.response.send_message("「カスタムVC」を購入しますか？ (30,000 Rune)", view=CustomRoomConfirmView(), ephemeral=True)
...
class PriceInputModal(discord.ui.Modal, title='部屋価格の設定'):
    price_input = discord.ui.TextInput(label='新しい価格 (Rune)', placeholder='例: 12000', max_length=10, required=True)
...
async def update_room_prices_config_view(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "**【変更手順】**\n"
            "下のドロップダウンメニューから、価格を変更したい部屋と時間を選択し、開いたモーダルで新しい価格（Rune）を入力してください。"
        ),
        color=discord.Color.blue()
    )
